...
$(function(){
    ...
    $("#newsAdd").submit(function(){
        ...
        if($title.val().length>20){
            $title.next("span").addClass("error").jsp("不得多于20字");
        }
        if($content.val().length>500){
            $content.next("span").addClass("error").jsp("不得多于500字");
        }
...
//            location.href = "Cart?action=addAddress&address="+$("#addAddr").val();
            ...
            var span=$("#span").jsp('<input type="text" name="addAddr" id="addAddr"/>');
            $(this).val("添加");
        }
    });
    //删除商品
    ...
    $("#guestBook").submit(function(){
        ...
        $(this).find("span").addClass("error").jsp("留言不得多于100字");
        return false;
    });
})